...
class TransEModel(object):
    ...
    def __init__(self, config):
        ...
        with tf.name_scope("embedding"):
            ...
            # why do we need negative value let the king become a king 
        ...
        if config.L1_flag:  # use L1-norm
            ...
            neg = tf.reduce_sum(abs(neg_h_e + neg_r_e - pos_t_e), 1, keep_dims = True)
            ...
            pos = tf.reduce_sum(abs(pos_h_e + pos_r_e - pos_t_e), 2, keep_dims = True)
            neg = tf.reduce_sum(abs(neg_h_e + neg_r_e - pos_t_e), 2, keep_dims = True)            
            # TO DO
